...
| **ttx follow-up** — partial fixes | ✅ | Landed two wins: **(a) swiglu fusion** — rewrote `build_moe_ffn_qwen36`, `build_shexp_qwen36`, and `build_swiglu_ffn` (qwen35) to emit `ggml_swiglu_split(gate, up)` instead of `ggml_silu(gate) * up`. This lets ggml-cuda's `ggml_cuda_should_fuse_mul_mat_vec_q` pattern-matcher fold the two gate/up `mul_mat_id` calls + silu + mul into ONE `mmvq` launch with `has_fusion=true`. nsys confirms our MXFP4 mmvq count dropped from 78 → 39 per step (matches llama.cpp) and Q8_0 from 201 → 121. **(b) persistent ggml ctx** — `build_step_graph` in both `test_generate` and `test_chain_spec` now calls `ggml_reset(ctx)` instead of `ggml_free` + `ggml_init`; tensors re-land at the same arena addresses so `cgraph->nodes[0]` is pointer-stable. `step_graph_free` keeps the ctx + alloc alive; a new `step_graph_destroy` is used at shutdown. Build time 550→292µs per step. Combined result on 35B MoE, 128 gen tokens, RTX 5090: test_generate AR 128 → 150 tok/s (+17%). |
| **ttx cache-layout refactor** — CUDA graphs fire | ✅ | Moved the KV cache to llama.cpp's 2-D `[n_embd_gqa, max_ctx]` layout, replaced `ggml_view + ggml_cpy` KV writes with `ggml_set_rows(cache, k_cur_2d, kv_pos_idx)` (kv_pos_idx is an input tensor updated per step), and padded the K/V read view + attention mask to `GGML_PAD(kv_len, 256)` so the view's `ne`/`data` don't shift step-to-step. With this, ggml-cuda's `ggml_cuda_graph_update_required` stops firing every step, `warmup_complete` actually latches, and subsequent steps become single `cudaGraphLaunch` calls instead of ~300 kernel launches apiece. nsys kernel-instance count on a 37-step test_generate run: **Q8_0 mmvq 7437 → 121** (essentially 1 capture + 36 graph-launched reuses). **Perf on RTX 5090, 35B MXFP4 MoE, 128 gen tokens from the fibonacci code prompt: test_generate AR 150 → 214 tok/s (+43%), which BEATS llama.cpp's 211 tok/s under the same bench.** test_chain_spec seq also jumps (78→98), tree_chain (81→108), ddtree K=4 B=12 (95→102). DDTree's gain is smaller because its per-round graph SHAPE varies with tree.n_nodes — follow-up to pad tree size to budget+1 for a stable shape. |
| **M4 re-run** — HE/GSM8K after cache-layout refactor | ✅ | Same `scripts/bench_chain_spec.py` driver, same 5-sample sweep, n_gen=256, N_spec=8 on RTX 5090. Every mode moved up vs the original M4 numbers (which ran on the pre-ttx binary with ~126 tok/s AR). `HumanEval`: AR 126→**212** (+69%), seq 73→113 (+55%), batch 121→**164** (+35%), tree_chain 121→**169** (+39%), ddtree K=8 B=22 105→120 (+14%). `GSM8K`: AR 126→**212** (+69%), seq 63→100 (+58%), batch 73→**116** (+60%), tree_chain 73→**123** (+69%), ddtree K=8 B=22 79→107 (+35%). **On both datasets tree_chain is now the best non-AR mode (0.80×AR HE, 0.58×AR GSM8K).** DDTree's relative disadvantage grew because draft catch-up (N_spec sequential 0.8B forwards per round) is the same absolute cost as before but now a larger fraction of each round's wall time — AR got much faster without a corresponding draft-side speedup. DDTree still wins on pathological low-AL prompts (e.g., GSM8K sample 2: batch 83 → DDTree 92, 51% sibling walks). No mode beats AR at these draft/target ratios; closing that gap needs M5. |
