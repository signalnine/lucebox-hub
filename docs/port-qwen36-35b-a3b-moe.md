...
| **M3a** — tree-mode chain smoke | ⏳ | `CHAIN_VERIFY=tree_chain` mode added: batched N-token verify via `ggml_gated_delta_net_tree_persist` + `ggml_ssm_conv_tree` with linear parent_ids = `[-1, 0, 1, ..., N-2]`. At N=4 the tree kernel is **byte-identical to sequential AR** (100 tok/s, lossless) — the small-T path uses the VEC kernels. At N≥8 it inherits the same fp16 MMA near-tie drift as `CHAIN_VERIFY=batch` on this prompt (both paths cross the same MMA threshold, same 0.022 logit flip). Cache-shape bug found and fixed: `create_target_cache` now takes `max_verify_tokens = N_spec` so the per-layer `conv_input_cache` tensor matches the in-graph concat shape `[(kern-1)+N_spec, conv_channels, 1]`. |
