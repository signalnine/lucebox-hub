...
| **M3b** — DDTree branching tree-verify | ✅ (slow) | `CHAIN_VERIFY=ddtree` mode: per-step top-K log-prob extraction from the draft, best-first branching tree build (`build_ddtree` ported from test_dflash), ancestor-only mask, target verify via the tree kernel, walk accepted path via target's posterior argmax. End-to-end **byte-identical to seq** on the code prompt at N=8 / K∈{1,4,8} / budget∈{8,12,22}. Currently **50 tok/s vs tree_chain's 135** because we catch up both caches every round via sequential replay of accepted tokens (no fast `ssm_intermediate`-based rollback yet). Branching didn't help AL on the code prompt — draft top-1 already matches target at every step. DDTree becomes a throughput win once M3c adds fast rollback (test_dflash already has the per-layer CUDA f16→f32 SSM restore + conv/target_feat compaction). Env knobs: `DDTREE_BUDGET`, `DDTREE_K`, `DDTREE_TEMP`. |
